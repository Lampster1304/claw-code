...
/// Write current worker state to `.agcli/worker-state.json` under the worker's cwd.
...
fn emit_state_file(worker: &Worker) {
    let state_dir = std::path::Path::new(&worker.cwd).join(".agcli");
...
mod tests {
    ...
    fn emit_state_file_writes_worker_status_on_transition() {
        ...
        let state_path = cwd_path.join(".agcli").join("worker-state.json");
        assert!(
            state_path.exists(),
