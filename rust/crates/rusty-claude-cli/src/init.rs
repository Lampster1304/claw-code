...
const STARTER_AGCLI_JSON: &str = concat!(
    "{\n",
    "  \"permissions\": {\n",
    "    \"defaultMode\": \"dontAsk\"\n",
    "  }\n",
...
const GITIGNORE_ENTRIES: [&str; 2] = [".agcli/settings.local.json", ".agcli/sessions/"];
...
pub(crate) fn initialize_repo(cwd: &Path) -> Result<InitReport, Box<dyn std::error::Error>> {
    ...
    let claw_dir = cwd.join(".agcli");
    artifacts.push(InitArtifact {
        name: ".agcli/",
        status: ensure_dir(&claw_dir)?,
    });

    let claw_json = cwd.join(".agcli.json");
    artifacts.push(InitArtifact {
        name: ".agcli.json",
        status: write_file_if_missing(&claw_json, STARTER_AGCLI_JSON)?,
    });
...
pub(crate) fn render_init_claude_md(cwd: &Path) -> String {
    ...
    lines.push("- Keep shared defaults in `.agcli.json`; reserve `.agcli/settings.local.json` for machine-local overrides.".to_string());
...
mod tests {
    ...
    fn initialize_repo_creates_expected_files_and_gitignore_entries() {
        ...
        assert!(rendered.contains(".agcli/"));
        assert!(rendered.contains(".agcli.json"));
        assert!(rendered.contains("created"));
        assert!(rendered.contains(".gitignore       created"));
        assert!(rendered.contains("CLAUDE.md        created"));
        assert!(root.join(".agcli").is_dir());
        assert!(root.join(".agcli.json").is_file());
        assert!(root.join("CLAUDE.md").is_file());
        assert_eq!(
            fs::read_to_string(root.join(".agcli.json")).expect("read agcli json"),
            concat!(
                "{\n",
                "  \"permissions\": {\n",
                "    \"defaultMode\": \"dontAsk\"\n",
        ...
        assert!(gitignore.contains(".agcli/settings.local.json"));
        assert!(gitignore.contains(".agcli/sessions/"));
    ...
    fn initialize_repo_is_idempotent_and_preserves_existing_files() {
        ...
        fs::write(root.join(".gitignore"), ".agcli/settings.local.json\n")
        ...
        assert!(second_rendered.contains(".agcli/"));
        assert!(second_rendered.contains(".agcli.json"));
        ...
        let gitignore = fs::read_to_string(root.join(".gitignore")).expect("read gitignore");
        assert_eq!(gitignore.matches(".agcli/settings.local.json").count(), 1);
        assert_eq!(gitignore.matches(".agcli/sessions/").count(), 1);
