...
fn slash_command_names_match_known_commands_and_suggest_nearby_unknown_ones() {
    ...
    // when
    let help_output = Command::new(env!("CARGO_BIN_EXE_agcli"))
        .current_dir(&temp_dir)
        .arg("/help")
        .output()
        .expect("agcli should launch");
    let unknown_output = Command::new(env!("CARGO_BIN_EXE_agcli"))
        .current_dir(&temp_dir)
        .arg("/zstats")
        .output()
        .expect("agcli should launch");
...
fn local_subcommand_help_does_not_fall_through_to_runtime_or_provider_calls() {
    ...
    assert!(doctor_stdout.contains("Usage            agcli doctor"));
    ...
    assert!(status_stdout.contains("Usage            agcli status"));
